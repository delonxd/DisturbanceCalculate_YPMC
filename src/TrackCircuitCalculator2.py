...
class Section(ElePack):
    ...
    def set_element(self):
        if self.m_type == '2000A':
            ...
            self.tcsr_posi = [self.j_length, self.m_length + self.j_length[0]]
...
class Line(ElePack):
    def __init__(self, sec_group=None, rail=None, train=None):
        name = sec_group.name_base + '线路'
        super().__init__(None, name)
        self.rail = Rail() if rail is None else rail
        self.element['区段组'] = sec_group
        self.element['列车'] = train
        self.ele_set = set()
        self.set_ele_set()

    def set_ele_set(self):
        pass
...
class SingleLineModel(ElePack):
    ...
    def get_model_element(self):
        # 全局元件
        ele_global = self.parent_ins.ele_set
        # 获取位置
        self.posi_line = self.get_posi_line(ele_global=ele_global, line=self.line)
        # 获取元件和钢轨段
    ...
    @staticmethod
    def get_posi_line(ele_global, line, ftype='元件'):
        posi_global = get_posi_fast(ele_global)
        posi_ele = get_posi_fast(line.ele_set)
        posi_rail = line['钢轨'].posi_list
        if ftype == '元件':
        ...
        posi_line = sort_posi_list(list(filter(lambda posit: rg[0] <= posit <= rg[-1], posi_global)))
        return posi_line
...
if __name__ == '__main__':
    ...
    # 轨道电路初始化
    tc1 = SectionGroup(name_base='主串', posi=0, m_num=1, freq1=2600,
